= Module 7: Optional Features for OpenShift 4.19/4.20/4.21
:page-layout: module

== OpenShift Virtualization [[virtualization]]

=== What is OpenShift Virtualization?
link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/virtualization[OpenShift Virtualization] enables you to run and manage virtual machines alongside containers in OpenShift Container Platform 4.19/4.20/4.21, providing a unified platform for both traditional and cloud-native workloads.

=== Key Features and Capabilities
* *VM Lifecycle Management*: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/virtualization[Create, manage, and delete virtual machines]
* *Live Migration*: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/virtualization[Migrate running VMs between nodes] without downtime
* *Storage Integration*: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/virtualization[Persistent storage for VM disks]
* *Network Configuration*: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/virtualization[Advanced networking options] including SR-IOV
* *Template Management*: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/virtualization[VM templates for standardized deployments]
* *GPU Passthrough*: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/virtualization[Direct GPU access for VMs]

=== Prerequisites for OpenShift Virtualization
Before installing OpenShift Virtualization, ensure:

* *Hardware Requirements*: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/virtualization[Nodes with hardware virtualization support]
* *CPU Features*: Intel VT-x or AMD-V virtualization extensions
* *Memory*: Sufficient RAM for both containers and virtual machines
* *Storage*: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/virtualization[ReadWriteMany (RWX) storage] for live migration
...
NOTE: Starting with OpenShift Virtualization 4.20, the HyperConverged operator manages most feature gates automatically based on cluster capabilities and OCP version. Best practice is to use a minimal CR and let the operator manage defaults. Only override specific options when you have a concrete reason. See the link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/virtualization/[OCP 4.21 Virtualization docs] for the current feature gate reference.

```yaml
# Create HyperConverged instance to enable virtualization
# Minimal recommended CR — operator manages feature gates and defaults
...
spec:
  liveMigrationConfig:
    allowAutoConverge: true           # Allow auto-converge for stuck migrations
...
The preferred way to create VMs is through the web console using Red Hat-provided templates. The YAML below shows the structure for reference. Note: `machine.type` should use a current QEMU machine type for your OCP version — use `q35` or the console default rather than a hard-coded version string.

```yaml
# Example RHEL 9 VM configuration (use RHEL 9 for new deployments)
...
spec:
  ...
  template:
    ...
    spec:
      domain:
        ...
        devices:
          ...
          - name: default
            masquerade: {}
        machine:
          type: q35                     # Use q35 - avoid hard-coded RHEL version strings
      networks:
      - name: default
        pod: {}
      volumes:
      - name: rootdisk
        containerDisk:
          image: quay.io/containerdisks/rhel:9-latest
      - name: cloudinitdisk
        cloudInitNoCloud:
          userData: |
            #cloud-config
            user: cloud-user
            # IMPORTANT: Do not hardcode passwords. Use SSH keys or a Secret reference.
            # Example: set password via: virtctl credentials set-password --user cloud-user --password <pw> rhel9-vm
            ssh_authorized_keys:
            - ssh-ed25519 AAAA... your-public-key-here
```

NOTE: For RHEL 8 VMs that still need to be created, replace the `image` with `quay.io/containerdisks/rhel:8-latest`. However, Red Hat recommends using RHEL 9 for new virtual machine deployments.
...
virtctl ssh cloud-user@rhel9-vm
```

=== Use Cases for OpenShift Virtualization
* *Legacy Application Migration*: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/virtualization[Migrate existing VMs to OpenShift]
...
For more information on images and permissions, see the link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/images/managing-images#images-allow-pods-to-reference-images-across-projects_using-image-pull-secrets[Image Guide]
...
For migrating VMs from VMware vSphere, use the link:https://docs.redhat.com/en/documentation/migration_toolkit_for_virtualization/2.10/html/installing_and_using_the_migration_toolkit_for_virtualization/index[Migration Toolkit for Virtualization]:

```bash
# Install Migration Toolkit for Virtualization
# Use channel release-v2.10 or the latest stable channel matching your needs
oc apply -f - <<EOF
...
link:https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.x/html/introduction_to_red_hat_openshift_ai/index[Red Hat OpenShift AI] provides a comprehensive platform for developing, training, serving, and monitoring AI/ML models on OpenShift Container Platform 4.19/4.20/4.21.

=== Key Features and Capabilities
* *Model Development*: link:https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.x/html/working_with_data_science_projects/index[Jupyter notebooks and development environments]
* *Model Training*: link:https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.x/html/working_with_data_science_pipelines/index[Distributed training with Kubeflow Pipelines]
* *Model Serving*: link:https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.x/html/serving_models/index[High-performance model inference serving]
* *MLOps Integration*: link:https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.x/html/working_with_data_science_pipelines/index[End-to-end ML pipeline automation]
* *GPU Support*: link:https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.x/html/installing_and_uninstalling_openshift_ai_self-managed/enabling-gpu-support_install[NVIDIA GPU acceleration for training and inference]
...
* *GPU Support*: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/hardware_accelerators[NVIDIA GPU Operator] for GPU acceleration
...
=== Installing OpenShift AI 3.x

NOTE: OpenShift AI 3.x introduced significant changes to the installation flow compared to 2.x. The operator name is now `rhods-operator` on the `rhoai` channel (or `fast-3.x` for early access to the latest 3.x features). A new **DSCInitialization** custom resource must be created before the DataScienceCluster. The `modelmeshserving` component has been removed from DataScienceCluster in 3.x (it is now managed separately). See the link:https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.x/html/installing_and_uninstalling_openshift_ai_self-managed/index[RHOAI 3.x installation guide].

==== Step 1: Install OpenShift AI Operator
```bash
# Create the namespace for RHOAI operator
oc create namespace redhat-ods-operator

# Install the OpenShift AI Operator on the rhoai channel (stable 3.x)
# Use fast-3.x to track latest 3.x patch releases
oc apply -f - <<EOF
apiVersion: operators.coreos.com/v1alpha1
kind: Subscription
metadata:
...
oc wait --for=condition=Ready pods -l name=rhods-operator -n redhat-ods-operator --timeout=120s
```

==== Step 2: Create DSCInitialization (Required in 3.x)
```yaml
# DSCInitialization must be created before DataScienceCluster
# This replaces the manual namespace/configmap setup from RHOAI 2.x
...
spec:
  ...
  trustedCABundle:
    managementState: Managed
  serviceMesh:
    managementState: Removed   # Set to Managed to enable ServiceMesh integration
```
...
oc wait --for=condition=Ready dscinitializations/default-dsci --timeout=180s
```

==== Step 3: Create DataScienceCluster
```yaml
# Create DataScienceCluster instance (RHOAI 3.x)
# NOTE: modelmeshserving is no longer a component of DataScienceCluster in 3.x
...
spec:
  components:
    ...
    kserve:
      managementState: Managed
      serving:
        managementState: Removed   # Set to Managed to deploy KNative-based model serving
    ray:
      managementState: Managed
    workbenches:
      managementState: Managed
...
spec:
  template:
    spec:
      containers:
      - name: ml-notebook
        image: quay.io/opendatahub/workbench-images:jupyter-datascience-c9s-py311_2024b_latest
        resources:
          requests:
            cpu: "1"
            memory: "4Gi"
...
link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/serverless[OpenShift Serverless] provides serverless computing capabilities:
...
link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/service_mesh[OpenShift Service Mesh] provides advanced traffic management:
...
link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/gitops[OpenShift GitOps] enables GitOps-based deployments:
...
link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/pipelines[OpenShift Pipelines] provides cloud-native CI/CD:
...
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/virtualization[Getting started with OpenShift Virtualization - OpenShift 4.21]
...
* link:https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.x/html/introduction_to_red_hat_openshift_ai/index[Introduction to Red Hat OpenShift AI]
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/serverless[About OpenShift Serverless - OpenShift 4.21]
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/service_mesh[Service Mesh 2.x - OpenShift 4.21]
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/gitops[GitOps - OpenShift 4.21]

== Workshop Conclusion [[conclusion]]

Congratulations! You've completed the OpenShift 4.19/4.20/4.21 Bare Metal Deployment Workshop. You now have comprehensive knowledge to:

=== Core Competencies Achieved
* *Deploy OpenShift 4.19/4.20/4.21* on bare metal infrastructure using multiple installation methods
...
* *Deploy Optional Features* such as OpenShift Virtualization and OpenShift AI 3.x
...
* *Certification*: Consider pursuing Red Hat OpenShift certifications (EX280 — Red Hat Certified Specialist in OpenShift Administration)
...
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/[OpenShift Container Platform 4.21 Documentation]
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/[OpenShift Container Platform 4.20 Documentation]
...
* link:https://docs.redhat.com/en/documentation/red_hat_openshift_data_foundation/4.21/[OpenShift Data Foundation 4.21 Documentation]
* link:https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.x/[OpenShift AI 3.x Documentation]
...
* link:https://access.redhat.com/support/policy/updates/openshift[Red Hat OpenShift Life Cycle Policy]
* link:https://access.redhat.com/support[Red Hat Customer Portal and Support]

Thank you for completing this comprehensive workshop on OpenShift 4.19/4.20/4.21 bare metal deployment!
