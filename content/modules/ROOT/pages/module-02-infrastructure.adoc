...
Based on link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/user-provisioned-infrastructure#installation-obtaining-installer_installing-bare-metal[official requirements]:
...
The link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/user-provisioned-infrastructure#installation-obtaining-installer_installing-bare-metal[pull secret] provides access to Red Hat container registries:
...
# For OpenShift 4.21 (recommended - latest supported):
wget https://mirror.openshift.com/pub/openshift-v4/x86_64/clients/ocp/stable-4.21/openshift-install-linux.tar.gz

# For OpenShift 4.20:
wget https://mirror.openshift.com/pub/openshift-v4/x86_64/clients/ocp/stable-4.20/openshift-install-linux.tar.gz

# For OpenShift 4.19:
wget https://mirror.openshift.com/pub/openshift-v4/x86_64/clients/ocp/stable-4.19/openshift-install-linux.tar.gz

# For OpenShift 4.18 (approaching EOL):
...
```bash
# Download oc CLI tool - pin to the same minor version as your target cluster
# Replace 4.21 with your target cluster version
wget https://mirror.openshift.com/pub/openshift-v4/x86_64/clients/ocp/stable-4.21/openshift-client-linux.tar.gz
...
Since link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/architecture/architecture-rhcos[RHCOS is an immutable operating system], we document hardware specifications using external methods. This component connects to both Standard HA and Compact cluster configurations.

=== Cluster Configuration Decision
Choose your cluster configuration based on link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/user-provisioned-infrastructure#minimum-resource-requirements_installing-bare-metal[official hardware requirements]:

```bash
# Document your cluster configuration choice
cat > cluster-config.txt << 'EOF'
# OpenShift 4.19/4.20/4.21 Cluster Configuration Choice
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/user-provisioned-infrastructure#minimum-resource-requirements_installing-bare-metal
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_a_single_node/preparing-to-install-sno#sno-requirements-for-installing-on-a-single-node_preparing-to-install-sno
...
Create comprehensive hardware inventory based on link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/user-provisioned-infrastructure#minimum-resource-requirements_installing-bare-metal[minimum resource requirements]:

```bash
# Create detailed hardware inventory
cat > hardware-inventory.txt << 'EOF'
# OpenShift 4.19/4.20/4.21 Hardware Inventory
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/user-provisioned-infrastructure#installation-user-infra-machines-iso_installing-bare-metal
...
DNS configuration is **mandatory** for OpenShift deployment. Based on link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/user-provisioned-infrastructure#installation-dns-user-infra_installing-bare-metal[official DNS requirements], choose your approach:

=== Option A: External DNS (Recommended for Production)
For production environments, configure external DNS based on link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/user-provisioned-infrastructure#installation-dns-user-infra_installing-bare-metal[DNS requirements]:

```bash
# Create DNS configuration documentation
cat > dns-config.txt << 'EOF'
# OpenShift 4.19/4.20/4.21 DNS Configuration
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/user-provisioned-infrastructure#installation-dns-user-infra_installing-bare-metal
...
# OpenShift 4.19/4.20/4.21 Network Configuration
...
Firewall rules are critical for OpenShift deployment. Based on link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/user-provisioned-infrastructure#installation-network-user-infra_installing-bare-metal[network requirements], configure these firewall rules:

==== Required Firewall Ports
```bash
# Create firewall configuration documentation
cat > firewall-config.txt << 'EOF'
# OpenShift 4.19/4.20/4.21 Firewall Configuration
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/user-provisioned-infrastructure#installation-network-user-infra_installing-bare-metal
#
# Note: OpenShift SDN was removed in OCP 4.20. OVN-Kubernetes is the only
# supported CNI plugin. The legacy VXLAN port (4789/udp) is NOT required.
...
22623/tcp   # Machine Config Server (internal cluster use only)
...
# Geneve (OVN-Kubernetes - the only supported CNI since OCP 4.20)
6081/udp    # Geneve overlay network (OVN-Kubernetes)
...
22623/tcp   # Machine Config Server (restrict to cluster internal network)
80/tcp      # Ingress health checks
443/tcp     # Ingress health checks
1936/tcp    # Ingress router health check endpoint (/healthz/ready)
EOF
...
# OpenShift 4.19/4.20/4.21 Boot Method Configuration
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/installer-provisioned-infrastructure
...
Network boot from PXE server based on link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/prepare-pxe-assets-agent[PXE assets preparation]:
...
NOTE: Starting with OpenShift 4.19, Secure Boot can be **enabled** on supported hardware (Dell PowerEdge Gen13+, HPE ProLiant Gen10+) using the `bootMode: UEFISecureBoot` option in `agent-config.yaml` or via the Assisted Installer console. For most standard deployments and all other hardware, Secure Boot should remain **disabled**. See the link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/user-provisioned-infrastructure[OCP 4.21 bare metal installation guide] for Secure Boot prerequisites.

```bash
# Configure on ALL nodes (standard deployment):
# ✅ UEFI mode enabled (required - Legacy BIOS not supported for new installs)
# ✅ Secure Boot: DISABLED for standard installs (see note above for exceptions)
...
#   Standard hardware:         Disabled (default, required)
#   Supported Dell/HPE (4.19+): Can enable via bootMode: UEFISecureBoot in agent-config.yaml
#   Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_bare_metal/

Node 1: [hostname]
☐ UEFI Mode: Enabled
☐ Secure Boot: Disabled (or enabled if using bootMode: UEFISecureBoot on supported HW)
