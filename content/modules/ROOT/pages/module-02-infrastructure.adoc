...
image::diagrams/network/openshift-firewall-architecture.svg[OpenShift Firewall Architecture,800,600]
...
For cloud-based bare metal or hybrid deployments:

```bash
# Document cloud firewall configurations
cat >> firewall-config.txt << 'EOF'
...
# - Inbound: 6443/tcp from 0.0.0.0/0 (or restricted CIDR)
# - Inbound: 22623/tcp from cluster CIDR
# - Inbound: 2379-2380/tcp from control plane security group
# - Inbound: 10250-10259/tcp from cluster CIDR
# - Outbound: All traffic

# Worker Security Group:
# - Inbound: 80/tcp from load balancer security group
# - Inbound: 443/tcp from load balancer security group
# - Inbound: 10250/tcp from control plane security group
# - Inbound: 30000-32767/tcp from load balancer security group
# - Outbound: All traffic

### Azure Network Security Groups:
# Similar rules adapted for Azure NSG format

### Google Cloud Firewall Rules:
# Similar rules adapted for GCP firewall format

### VMware NSX:
# Distributed firewall rules for on-premises VMware environments
EOF

echo "✅ Cloud provider firewall examples documented"
```
