...
* **Installation with Customizations**: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/installing-with-agent-based-installer[Installing a cluster with customizations] - Step-by-step installation guide
* **Disconnected Installation**: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/understanding-disconnected-installation-mirroring[Understanding disconnected installation mirroring] - Air-gapped deployment guide
...
```yaml
# install-config.yaml - Agent-based Installer Configuration (Standard HA Cluster)
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/preparing-to-install-with-agent-based-installer#installation-bare-metal-agent-installer-config-yaml_preparing-to-install-with-agent-based-installer
apiVersion: v1
...
- architecture: amd64                      # Supported: amd64, arm64, ppc64le, s390x
  ...
  name: worker
  replicas: 2                             # Number of worker nodes (2+ for Standard HA)
controlPlane:
  architecture: amd64
  hyperthreading: Enabled
  name: master
  replicas: 3                             # 3 control plane nodes for Standard HA
metadata:
  name: ocp-cluster                       # Cluster name (must be DNS-compliant)
networking:
  clusterNetwork:
  - cidr: 10.128.0.0/14                   # Pod network CIDR block
    hostPrefix: 23                        # Subnet prefix per node
  machineNetwork:
  - cidr: 192.168.1.0/24                  # Machine network CIDR
  networkType: OVNKubernetes              # Only supported network plugin (SDN removed in 4.20)
  serviceNetwork:
  - 172.30.0.0/16                         # Service network CIDR
platform:
  baremetal:                              # Use baremetal platform for VIP management
    apiVIPs:
...
fips: false                               # Set to true for FIPS 140-2/140-3 compliance
```

NOTE: For a **Compact cluster** (3 nodes, control plane + worker combined), set `compute.replicas: 0` and `controlPlane.replicas: 3`. For **Single-Node OpenShift (SNO)**, set `compute.replicas: 0`, `controlPlane.replicas: 1`, and use `platform: none: {}`. See the link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/[OCP 4.21 Agent-based Installer docs] for all topology options.
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/preparing-to-install-with-agent-based-installer#agent-host-config_preparing-to-install-with-agent-based-installer
apiVersion: v1beta1
kind: AgentConfig
metadata:
  name: sno-cluster
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/preparing-to-install-with-agent-based-installer#agent-install-networking-DHCP_preparing-to-install-with-agent-based-installer
apiVersion: v1beta1
kind: AgentConfig
metadata:
  name: sno-cluster
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/preparing-to-install-with-agent-based-installer#agent-install-networking-static_preparing-to-install-with-agent-based-installer
apiVersion: v1beta1
kind: AgentConfig
metadata:
  name: sno-cluster
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/preparing-to-install-with-agent-based-installer#agent-install-sample-config-bonds-vlans_preparing-to-install-with-agent-based-installer
apiVersion: v1beta1
kind: AgentConfig
...
For air-gapped environments, the Agent-based Installer requires a local mirror registry containing all necessary OpenShift images. This section covers the link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/understanding-disconnected-installation-mirroring[disconnected installation mirroring] process using the link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/disconnected_environments/mirroring-in-disconnected-environments#installing-mirroring-creating-registry[mirror registry for Red Hat OpenShift].

[NOTE]
====
The **mirror registry for Red Hat OpenShift** is a small-scale container registry included with OpenShift Container Platform subscriptions. It provides a streamlined way to create a local mirror of OpenShift images for disconnected installations. The tool is available from the link:https://console.redhat.com/openshift/downloads#tool-mirror-registry[OpenShift Console Downloads page] and as open source at link:https://github.com/quay/mirror-registry[GitHub].
====
...
* **Mirror Registry Setup**: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/disconnected_environments/mirroring-in-disconnected-environments#mirror-registry-localhost_installing-mirroring-creating-registry[Mirror registry for Red Hat OpenShift installation] - Official mirror registry setup guide
...
* **oc-mirror Plugin**: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html-single/disconnected_environments/#about-installing-oc-mirror-v2[Mirroring with oc-mirror plugin v2] - Advanced mirroring tool
* **Mirror Registry Downloads**: link:https://console.redhat.com/openshift/downloads#tool-mirror-registry[OpenShift Console Downloads] - Official download location for mirror-registry tool

=== Setting Up Mirror Registry

Establish a local container registry for storing mirrored OpenShift images using the link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/disconnected_environments/mirroring-in-disconnected-environments#mirror-registry-localhost_installing-mirroring-creating-registry[mirror registry for Red Hat OpenShift]:
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/disconnected_environments/mirroring-in-disconnected-environments#mirror-registry-localhost_installing-mirroring-creating-registry
...
# Set environment variables - update OCP_RELEASE to match your target cluster version
export OCP_RELEASE="4.21.0"
...
# The command will output imageDigestSources configuration (formerly imageContentSources)
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/understanding-disconnected-installation-mirroring#configuring-the-agent-based-installer-to-use-mirrored-images
apiVersion: v1
...
additionalTrustBundle: |                  # Mirror registry CA certificate
  -----BEGIN CERTIFICATE-----
  <mirror-registry-certificate-content>
  -----END CERTIFICATE-----
# NOTE: imageDigestSources replaces the deprecated imageContentSources field.
# Use the output from 'oc adm release mirror' or 'oc mirror' to populate this section.
# imageContentSources still works for backward compatibility but generates a deprecation warning.
imageDigestSources:
- mirrors:
...
For advanced mirroring scenarios, use the link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html-single/disconnected_environments/#about-installing-oc-mirror-v2[oc-mirror plugin v2]:
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html-single/disconnected_environments/#installing-oc-mirror-v2
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/installing-with-agent-based-installer#installing-ocp-agent-boot_installing-with-agent-based-installer
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/prepare-pxe-assets-agent
...
# Reference: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/installing-with-agent-based-installer#installing-ocp-agent-verify_installing-with-agent-based-installer
...
# master-0   Ready    control-plane,master   10m   v1.34.0+...
...
* **Primary Guide**: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/preparing-to-install-with-agent-based-installer[Preparing to install with the Agent-based Installer] - Complete preparation guide
* **Installation Process**: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/installing-with-agent-based-installer[Installing a cluster with customizations] - Step-by-step installation
* **Disconnected Environments**: link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_an_on-premise_cluster_with_the_agent-based_installer/understanding-disconnected-installation-mirroring[Understanding disconnected installation mirroring] - Air-gapped deployment guide
